...
[@bs.module]
external flatten: Js.Array.t(Js.Array.t('a)) => Js.Array.t('a) =
  "lodash.flatten";

[@bs.module]
external shuffle: Js.Array.t('a) => Js.Array.t('a) = "lodash.shuffle";

[@bs.module "twemoji"] [@bs.scope "convert"]
...
let mapToProfessions = (professions, people) => {
  professions
  |> Js.Array.map(
       profession => Js.Array.map(person => person ++ joiner ++ profession),
       people,
     )
  |> flatten;
};
...
let mapToSkinTones = people =>
  ...
  |> Js.Array.map(profession => {
       Js.Array.map(tone => profession ++ tone);
       skinTones;
     })
  |> flatten;
...
let mapToGenders = people =>
  people
  |> Js.Array.map(person => {
       Js.Array.map(gender => person ++ joiner ++ gender);
       genders;
     })
  |> flatten;
let professions = [|
